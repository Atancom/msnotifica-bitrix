...
if (!$in || empty($in['filename']) || empty($in['mimetype']) || empty($in['content_base64'])) {
  http_response_code(400); exit('missing json fields');
}

// Guardar a temp
$tmp = tempnam(sys_get_temp_dir(), 'ocr_');
file_put_contents($tmp, base64_decode($in['content_base64']));

// Enviar a OCR.Space como archivo
...
$post = [
  ...
  'isTable' => 'true',
  'file' => new CURLFile($tmp, $in['mimetype'], $in['filename']),
];
